...
#include <vector>
  ...
  for (int i=1;i<Nevents; i++)
    //  for (int i=0;i<10; i++) 
    {
      ...
      file = "/pc/choozdsk01/palomare/SiPM/TT_FBK_05/DC/OV25-sec-50-50us--";
      //      if ( i==35 ) continue;
      ...
      for (int ij=0;ij<Nsegs; ij++)
	{
	  ...
	  //	  while (j<Nunits)
	      ...
	      // Pedestal info 
	      if ( j<20 ) mean_ped_i += Vadc[j];
	      if ( j==20 ) {
		mean_ped_i /=20.;
	      }
	      if ( j>480 && j<500 ) mean_ped_f += Vadc[j];
	      if ( j==500 ) mean_ped_f /=20.;

	      // High-pass filtering
	      if ( f==0 ) {f=Vadc[j]; f_old=f;}                        
              else                                                     
                {                                                      
	      ...
	      if ( (f<-0.003 && sum_time<100.) || (sum_time>0 && sum_time<100.) ) // Threshold and timing should be tuned
	      //	      if ( (f<-0.003 && sum_time<200.) || (sum_time>0 && sum_time<100.) ) // Threshold and timing should be tuned
		{
		  
		  if ( f<adc_min ) {
		    adc_min=f; time_min=Dt[ij]*1.e+9+DeltaT*(j+0.5);
		  }
		...
		} else if ( sum_time>=100. )
		{

		  peakA[p]=adc_min;
		  peakT[p]=time_min;
		  if (p>0) Delta[p]=time_min-peakT[p-1];
